...
| 項目 | 仕様 |
|------|------|
| 切り出し方法 | **AIハイライト検出**（推奨）/ 手動で開始・終了時間を指定 |
| 切り出し長さ | 最大55秒（オープニング5秒 + 本編55秒 = 60秒） |
| プレビュー | 切り出し範囲のプレビュー表示 |

#### AIハイライト検出機能 🆕
元動画を分析して、YouTubeショートとして「バズりそうな」部分を自動で検出・提案する。

| 項目 | 仕様 |
|------|------|
| 分析方法 | 音声文字起こし + LLM分析（GPT-4） |
| 検出基準 | 盛り上がり、面白い発言、インパクト、話題性 |
| 提案数 | 最大5つのハイライト候補を提示 |
| 出力 | 各候補の開始時間・終了時間・理由・スコア |
...
│         ↓                                                   │
│  4. LLM分析（GPT-4）                                        │
│     「この文字起こしから、YouTubeショートとして              │
│      バズりそうな55秒の区間を5つ提案してください」          │
│         ↓                                                   │
│  5. ハイライト候補を表示                                     │
│     ┌──────────────────────────────────────────┐            │
│     │ 候補1: 03:24 - 04:19 ⭐⭐⭐⭐⭐           │            │
